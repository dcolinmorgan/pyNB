...
def process_single_run(snr, id, method, zetavec, nb):
    try:
        dataset = Data.from_json_url(
            "https://bitbucket.org/sonnhammergrni/gs-datasets/raw/d2047430263f5ffe473525c74b4318f723c23b0e/N50/Tjarnberg-ID"
            + id
            + "-D20151111-N50-E150-SNR"
            + snr
            + "-IDY"
            + id
            + ".json"
        ...
        true_net = Network.from_json_url(
            "https://bitbucket.org/sonnhammergrni/gs-networks/raw/0b3a66e67d776eadaa5d68667ad9c8fbac12ef85/"
            + dataset.data.network.network.split("-")[2]
            + "/N50/"
            + dataset.data.network.network
            + ".json"
        )
