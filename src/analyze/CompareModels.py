...
class CompareModels:
    ...
    def __init__(
        self, ref_net: Network, net_list: np.ndarray | Network | list[Network]
    ) -> None:
        ...
        if isinstance(net_list, Network):
            if net_list.A is not None and net_list.A.ndim == 3:
                ...
                self._models = [
                    Model(Network(net_list.A[:, :, i]))
                    for i in range(net_list.A.shape[2])
                ]
            else:
            ...
            if net_list.ndim == 3:
                self._models = [
                    Model(Network(net_list[:, :, i])) for i in range(net_list.shape[2])
                ]
            else:
            ...
            self._models = [
                Model(net) if isinstance(net, Network) else Model(Network(net))
                for net in net_list
            ]
    ...
    def _analyze(self) -> None:
        ...
        for i, model in enumerate(self._models):
            ...
            # Rate metrics
            self._sen[i] = (
                self._TP[i] / (self._TP[i] + self._FN[i])
                if (self._TP[i] + self._FN[i]) > 0
                else 0
            )
            self._spe[i] = (
                self._TN[i] / (self._TN[i] + self._FP[i])
                if (self._TN[i] + self._FP[i]) > 0
                else 0
            )
            self._comspe[i] = 1 - self._spe[i]
            self._pre[i] = (
                self._TP[i] / (self._TP[i] + self._FP[i])
            ...
            # F1 score
            self._F1[i] = (
                2 * (self._pre[i] * self._sen[i]) / (self._pre[i] + self._sen[i])
                if (self._pre[i] + self._sen[i]) > 0
                else 0
            )
            ...
            self._FEL[i] = (
                self._FP[i] / np.sum(ref_binary) if np.sum(ref_binary) > 0 else 0
            )
